            ...
            {demoTracks.map((track, index) => (
              <div
                ...
                className={`flex items-center gap-4 p-3 rounded-lg transition-colors group cursor-pointer ${
                  currentTrack?.id === track.id ? 'bg-purple-500/15' : 'hover:bg-white/5'
                }`}
              >
                <div className="w-8 text-gray-400 text-sm">{index + 1}</div>
                <button
                  onClick={() => playTrack(track.id)}
                  className="w-10 h-10 rounded-full bg-purple-500/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                >
                ...
                <div className="text-sm text-gray-400 w-16 text-right">{track.durationLabel}</div>
